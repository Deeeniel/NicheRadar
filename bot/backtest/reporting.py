...
from bot.backtest.dataset import BacktestSample, samples_to_dicts
from bot.backtest.metrics import build_backtest_metrics
...
def _markdown(report: dict[str, object]) -> str:
    ...
    lines = [
        ...
        f"- Model Alpha (Brier Diff): `{_fmt(summary.get('model_alpha'))}`",
    ...
    lines.extend(
        [
            "",
            "## Combinatorial Buckets (Evidence & Net Edge)",
            "",
            "| evidence | net edge | count | fills | avg pnl | win rate |",
            "| --- | --- | ---: | ---: | ---: | ---: |",
        ]
    )
    for row in _list(report.get("combinatorial_buckets")):
        lines.append(
            f"| {row.get('evidence_bucket')} | {row.get('net_edge_bucket')} | "
            f"{row.get('count')} | {row.get('fills')} | "
