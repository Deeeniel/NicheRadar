...
def build_backtest_metrics(samples: list[BacktestSample], min_samples: int = 20) -> dict[str, object]:
    ...
    return {
        "summary": {
            ...
            "model_alpha": _round(market_mid_brier_score(samples) - brier_score(samples)) if market_mid_brier_score(samples) is not None and brier_score(samples) is not None else None,
        ...
        "combinatorial_buckets": group_combinatorial(samples),
...
def recommendations(samples: list[BacktestSample], min_samples: int) -> list[str]:
    settled_samples = [sample for sample in samples if sample.target_source == "settlement_file"]
    settled_count = len(settled_samples)
    if settled_count < max(30, min_samples):
        return [
    ...
    for profile, group in _groups(settled_samples, "model_profile").items():
        if len(group) >= 10:
            avg_pnl = _avg([float(s.realized_pnl) for s in group if s.realized_pnl is not None])
            if avg_pnl is not None:
                if avg_pnl < 0:
                    recs.append(f"Auto-Tune [{profile}]: Marginal PnL ({avg_pnl:.4f}). Consider lowering base_logit or increasing minimum confidence thresholds.")
                elif avg_pnl > 0.2:
                    recs.append(f"Auto-Tune [{profile}]: Strong positive PnL ({avg_pnl:.4f}). Validation successful. Consider increasing evidence_weight.")
...
def group_combinatorial(samples: list[BacktestSample]) -> list[dict[str, object]]:
    bucketed: dict[tuple[str, str], list[BacktestSample]] = defaultdict(list)
    for sample in samples:
        e_bucket = _bucket(sample.evidence_score)
        n_bucket = _bucket(sample.net_edge)
        bucketed[(e_bucket, n_bucket)].append(sample)

    rows: list[dict[str, object]] = []
    for (e_bucket, n_bucket), group in sorted(bucketed.items()):
        rows.append(
            {
                "evidence_bucket": e_bucket,
                "net_edge_bucket": n_bucket,
                "count": len(group),
                "fills": len([sample for sample in group if sample.fill_eligible]),
                "avg_p_model": _avg([sample.p_model for sample in group if sample.p_model is not None]),
                "observed_yes_rate": _avg([sample.target_yes_probability for sample in group if sample.target_yes_probability is not None]),
